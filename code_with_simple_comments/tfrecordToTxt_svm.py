...
#####################################################################################################
		...
		mapclass = {0: 0, 1: 1, 3: 2, 8: 3, 10: 4, 12:5, 18:6, 22:7, 23:8, 25:9} # map table
        #new_vid_ids = []
        new_labels = []
        new_mean_rgb = []
        new_mean_audio = []
...
#####################################################################################################
		...
		# use next 3 lines if producing svm labels
        with open('svm_'+ dataType +'_labels.txt', 'w') as labelfile:
...
#####################################################################################################
# main: call tftoTxt wrt train, validate and test dataset
tftoTxt("train")
tftoTxt("validate")
tftoTxt("test")
